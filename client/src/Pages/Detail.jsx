...
function Detail() {
  ...
  return (
    <section>
      ...
      <ul>
        {country.activities.map(({id,name,difficulty,duration,saeason})=>(<li key={id}>
          <h3>{name}</h3>
          <p>{`Esta actividad tiene una dificultad de ${difficulty}, y para realizar esta se requiere un tiempo de ${duration} horas y se puede realizar en la temporada de ${saeason??"Verano"}`}</p>
          </li>))}
      </ul>
      :
